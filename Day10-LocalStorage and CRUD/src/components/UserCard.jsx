...
const UserCard = ({ user, deleteUser, setToggle, setUpdatedUser }) => {
  return (
    <>
      {user.map((u, index) => {
        const { name, email, contact, imageURL } = u;
        return (
          <div
            key={index}
            className="border p-2 m-2 rounded-xl w-[18vw] h-[45vh] flex flex-col justify-between"
          >
